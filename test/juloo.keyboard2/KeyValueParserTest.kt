...
class KeyValueParserTest {
    ...
    @Test
    fun parse_macro() {
        Utils.parse("copy:ctrl,a,ctrl,c", KeyValue.makeMacro("copy", arrayOf(
            KeyValue.getSpecialKeyByName("ctrl"),
            KeyValue.makeStringKey("a"),
            KeyValue.getSpecialKeyByName("ctrl"),
            KeyValue.makeStringKey("c")
        ), 0))
        Utils.parse("macro:abc,\\'", KeyValue.makeMacro("macro", arrayOf(
        ...
        Utils.parse("<2:ctrl,backspace", KeyValue.makeMacro("<2", arrayOf(
            KeyValue.getSpecialKeyByName("ctrl"),
    ...
    @Test
    fun parse_string_key() {
        Utils.parse("symbol:'str'", KeyValue.makeMacro("symbol", arrayOf(
            KeyValue.makeStringKey("str")
        ), 0))
        Utils.parse("symbol:'str\\''", KeyValue.makeMacro("symbol", arrayOf(
            KeyValue.makeStringKey("str'")
        ), 0))
        Utils.parse("macro:'str',abc", KeyValue.makeMacro("macro", arrayOf(
            KeyValue.makeStringKey("str"),
            KeyValue.makeStringKey("abc")
        ), 0))
        Utils.parse("macro:abc,'str'", KeyValue.makeMacro("macro", arrayOf(
            KeyValue.makeStringKey("abc"),
            KeyValue.makeStringKey("str")
        ), 0))
        Utils.parse("macro:\\',\\,", KeyValue.makeMacro("macro", arrayOf(
            KeyValue.makeStringKey("'"),
            KeyValue.makeStringKey(",")
        ), 0))
        Utils.parse("macro:a\\'b,a\\,b,a\\xb", KeyValue.makeMacro("macro", arrayOf(
    ...
    fun parse_key_event() {
        ...
        Utils.parse("macro:keyevent:85,abc", KeyValue.makeMacro("macro", arrayOf(
            KeyValue.keyeventKey("", 85, 0),
            KeyValue.makeStringKey("abc")
        ), 0))
        Utils.parse("macro:abc,keyevent:85", KeyValue.makeMacro("macro", arrayOf(
            KeyValue.makeStringKey("abc"),
            KeyValue.keyeventKey("", 85, 0)
        ), 0))
    ...
    fun parse_old_syntax() {
        ...
        Utils.parse(":str symbol='Symbol' flags='dim':'f'", KeyValue.makeStringKey("f").withSymbol("Symbol").withFlags(KeyValue.FLAG_SECONDARY or KeyValue.FLAG_SMALLER_FONT))
        Utils.parse(":str flags='dim,small':'Foo'", KeyValue.makeStringKey("Foo", KeyValue.FLAG_SECONDARY or KeyValue.FLAG_SMALLER_FONT))
    ...
    object Utils {
        fun parse(key_descr: String, ref: KeyValue) {
            assertEquals(ref, KeyValue.getKeyByName(key_descr))
        }

        fun expect_error(key_descr: String) {
            try {
